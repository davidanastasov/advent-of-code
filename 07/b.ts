...
const MIN_FREE_STORAGE = 30000000;

const usedStorage = directorySizes.sort((a, b) => b.size - a.size)[0].size;
const validDirectories = directorySizes.filter(
  (dir) => TOTAL_STORAGE - usedStorage + dir.size >= MIN_FREE_STORAGE
);

const smallestSize = validDirectories.sort((a, b) => a.size - b.size)[0].size;
